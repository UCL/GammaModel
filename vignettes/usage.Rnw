...
\begin{figure}
<<gml2,fig.asp=0.6,eval=TRUE,comment=NA>>=
# load data
data(Neolithic) 
...
\begin{figure}
<<mcmc1,fig.asp=0.6,eval=TRUE,cache=TRUE,comment=NA>>=
...
\begin{figure}
<<mcmc3,eval=TRUE,cache=TRUE,comment=NA,dependson='mcmc1'>>=
...
# add 1000 samples from the chain
for(n in 1:1000){
