...
The GammaModel package also provides a number of age-class models, each of which is described by the probability of death occuring in age classes A to I as follows: 
...
P-values are estimated by sampling 10,000 arrangements of the data, but for improved precision this default value may be increased by changing the N argument:
<<gof5, eval=FALSE>>=
